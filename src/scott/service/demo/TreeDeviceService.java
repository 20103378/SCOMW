...
import scott.dao.demo.TreeDeviceDao;
...
public class TreeDeviceService<T,T1,T2,T3,T4,T5,T6> extends BaseService<T>{
	private final static Logger log= Logger.getLogger(TreeDeviceService.class);
...
/*	@Service("baseTbldevicechannelService")
    ...
    public List<T> getOtherImgListBySelect(String SampleTime,String Space,String DeviceType){
        return getDao().getOtherImgListBySelect(SampleTime,Space,DeviceType);
    }

	public List<T> getZoneEmuList(SpaceEntity space)
	{
		List<T> t1=getDao().getZoneEmuList(space);
		return t1;
	}
