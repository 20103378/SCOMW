...
public class LEDConfigurationController extends BaseAction {
	...
	private final static Logger log = Logger
			.getLogger(LEDConfigurationController.class);

	// Servrice start
	@Autowired(required = false)
	// 自动注入，不需要生成set方法了，required=false表示没有实现类，也不会报错。
	private LEDConfigurationService<TreeDeviceEntity> LEDService;
//	public LEDConfigurationService<TreeDeviceEntity> getLEDService(){
//		return LEDService;
//	}
	@RequestMapping("/list")
	...
	public void getOsicfgXml(HttpServletResponse response,HttpServletRequest request) throws Exception {
		...
		int a=LEDService.del_yc_instAll();
		if(a==1){
	...
	public void commitled(OsicfgEntity entity,
			...
			Element root = document.getRootElement();		//获取根节点对象
			List<Element> Networklist = root.getChildren("NetworkAddressing");
			int flag=LEDService.updateFlag(entity);
			if(flag==1){
			//开始修改xml
				for(Element el: Networklist){
					List<Element> RemoteAddressList= el.getChildren("RemoteAddressList");
