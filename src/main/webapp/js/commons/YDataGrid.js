//var YDataGrid = function(config,table_name){
//config:datagrid内容        table_name:jsp中datagrid所属table的id toolbarInit:菜单栏是否默认 titleInit:标题是否初始化 singleSelect:是否多选 
var YDataGrid = function(config,table_name,toolbarInit,titleInit,singleSelect,pagination){
		config = config || {};
...
//				}
			...
			if(!titleInit){
				delete dataconfig.title;
				
			}
			//alert(JSON.stringify(dataconfig))
			Grid.datagrid(dataconfig);
		}
		//初始化Grid按钮 按钮控制
		var initTbar = function(){
			var tbars = getToolbar();
			var _url = ctxPath + '/getActionBtn.do';
