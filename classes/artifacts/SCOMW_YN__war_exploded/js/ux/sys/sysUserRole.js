		...
		init:function(){
			$("#roleIds").combobox({
				...
				formatter:function(row){
				  var s = "<span><input type='hidden'  class='selectId' style='vertical-align: middle' id='selectId_"+row.id+"'>"+row.roleName+"<span>"
				  return s;  
				},
				onSelect:function(record){
					$("#selectId_"+record.id).attr("selected", true);
				},
				onUnselect:function(record){
					$("#selectId_"+record.id).attr("checked", false);
				}
			});
			_box = new YDataGrid(_this.config,"#data-list",true,true,false,true); 
			_box.init();
		}
	}
	return _this;
