...
//    	var month=curr_time.getMonth();
    	...
    	$.ajax({
        	...
        	cache: false,
        	type: 'POST',
        	url: ctxPath + '/treeDevice/getImgList.do',
...
function linkToData(devId,name,type) {
	var url=encodeURI(ctxPath + '/deviceHealthState/DeviceDetail.shtml?DeviceID='+devId+'&DeviceType='+type+'&DeviceName='+name);
...
function showdata(id, data ,otherData,remarkData){
    ...
    for (var i = 0, l = data.length; i < l; i++) {
    	...
    	if(ln==null){
    		...
    		html1 +="</div>";
    		html1 +="</div>";
    	    html1 +="<div id='div"+n+"' style='width:100%; overflow:hidden; background:#f8f8f8;'>";
    	    n++;
    	}else if(ln!=ln_name){
    		ln=ln_name;
    		if(ln=="1"){
    		...
    		if(trClass%2==1){
    			...
    			html1 +="</div>";
    			html1 +="<div style='width:100%;overflow:hidden;'>";
    			html1+="<div style='float:left;width: 300px; text-align:left;padding-left:20px;overflow:hidden;'>";
        		html1 +="<p style='float:left;font-family:Microsoft YaHei;font-size:20;font-weight:bold'>"+group+"</p>";
    			if(group!="红外测温" && group!="辅助设备"){
    				html1 +="<a href='javascript:void(0);' style='display: block; width: 70px; margin: 25px 180px;' class='easyui-linkbutton l-btn' iconcls='icon-search' onclick='toSpaceData("+ln+")' value='设备报表'>";
    				html1 +="<span class='l-btn-left'><span class='l-btn-text icon-search l-btn-icon-left'>报表</span></span></a>";
        		...
        		html1 +="</div>";
    			html1 +="</div>";
    		    html1 +="<div id='div"+n+"' style='width:100%;overflow:hidden;'>";
    		}
    		n++;
    	}
    	imgvalue="0";
 ...
 //   	   	var dateTime1 = year + "-" + month + "-" + day +" "+ hour +":"+minute+":"+second;
        ...
        if(otherData.length>0){
	        for(var ii =0;ii<otherData.length;ii++){
	        	if(data[i].id.indexOf("C")>-1){
	        		if(data[i].text==otherData[ii].text){
	        			imgvalue_amc="0";
		        	}
	        	}else{
		        	if(data[i].text==otherData[ii].text
//		        			||data[i].nodetype=='hwcw'
		        				){
		        		imgvalue="0";
		        	}
	        	}
        	...
        	if(data[i].nodetype=='hwcw'){
        		imgvalue="0";
        	}
        }
        for(var ii =0;ii<remarkData.length;ii++){
        	if(data[i].id==remarkData[ii]){
//        		alert("data[i].id="+data[i].id+"||remarkData[ii]="+remarkData[ii]);
        		imgvalue="2";
        	}
        };
        if(data[i].id.indexOf("C")>-1){
        ...
        var showwin_id=otherData[i].id;
        var showwin_type=otherData[i].nodetype;
		...
		html1+="<div id='"+otherData[i].id+"' class='tt-inner' style='margin:1px;background:#8acff0;float:left;width: 500px; height: 50px; text-align:center; overflow:hidden; border:1px' onclick='toCurrentData("+data_Current+")' >" +
				...
				"<img id='tt' src=" + img_path + " title=" + name_title + " style='float:right;'/>" +
					"</img>" +
				"</div>";
		imgname="";
...
//		var DeviceState=$("#DeviceState").combo("getValue");
		...
		$.ajax({
	    	...
	    	url: ctxPath + "/treeDevice/getImgListBySelect.do?Space="+DeviceArea+"&DeviceType="+DeviceType,
	    	error: function(){// 请求失败处理函数
	    		jeecg.alert('错误信息', '数据请求失败', 'error');
	    	},
	    	success: function(data){
...
function toCurrentData(data){
	...
	if(id.indexOf('C')>-1){
		...
		var url=encodeURI(ctxPath + '/deviceHealthState/DeviceDetail.shtml?DeviceID='+id+'&DeviceType='+nodetype+'&DeviceName='+nodetxt_DeviceDetail);
		
	}else{
		var url=encodeURI(ctxPath + '/deviceHealthState/DeviceDetail.shtml?DeviceID='+id+'&DeviceType='+nodetype+'&DeviceName='+nodetxt);
	}
...
function toSpaceData(data){
	...
	var url=encodeURI(ctxPath + '/deviceHealthState/SpaceDetail.shtml?DeviceType='+data+'&DeviceName='+nodetxt);
	parent.jeecg.main.addTab(nodetxt,url);
}
